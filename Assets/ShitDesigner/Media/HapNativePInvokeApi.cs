...
namespace ShitDesigner.Media
{
    ...
    public sealed class HapDecodedFrame
    {
        ...
        public HapDecodedFrame(uint width, uint height, ulong frameIndex, ulong presentationTicks, byte[] rgba, HapDecodedPlane[] planes, bool usesCpuFallback = true, bool isYCoCg = false)
        { Width = width; Height = height; FrameIndex = frameIndex; PresentationTicks = presentationTicks; Rgba8PremultipliedLinear = rgba; Planes = planes ?? Array.Empty<HapDecodedPlane>(); UsesCpuFallback = usesCpuFallback; IsYCoCg = isYCoCg; }
    }
    ...
    /// <summary>Result of loading the installed native binary and querying its
    ...
    public sealed class PInvokeHapNativeApi : IHapNativeApi, IHapNativePreparedApi
    {
        ...
        public Result<IntPtr> Open(VideoPrepareRequest request)
        {
            ...
            try
            {
                ...
                return Result<IntPtr>.Success(handle);
            }
        ...
        public Result Play(IntPtr handle) => Call(() => sd_hap_play(handle), "media.hap.play_failed");
        public Result Pause(IntPtr handle) => Call(() => sd_hap_pause(handle), "media.hap.pause_failed");
        public Result Stop(IntPtr handle) => Call(() => sd_hap_stop(handle), "media.hap.stop_failed");
        public Result SetSpeed(IntPtr handle, double speed) => Call(() => sd_hap_set_speed(handle, speed), "media.hap.speed_failed");
        public Result SetLoop(IntPtr handle, bool loop) => Call(() => sd_hap_set_loop(handle, loop ? 1 : 0), "media.hap.loop_failed");
        public Result Seek(IntPtr handle, double seconds) => Call(() => sd_hap_seek(handle, seconds), "media.hap.seek_failed");
        public Result SyncToGraphClock(IntPtr handle, double logicalSeconds, bool demanded) => Call(() => sd_hap_sync(handle, logicalSeconds, demanded ? 1 : 0), "media.hap.clock_failed");
        ...
        public Result<HapDecodedFrame> AcquireFrame(IntPtr handle, long index)
            => AcquireFrameCore(handle, index, compressedOnly: false);

        private Result<HapDecodedFrame> AcquireFrameCore(IntPtr handle, long index, bool compressedOnly)
        {
            ...
            lock (_gate)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        return Result<HapDecodedFrame>.Success(new HapDecodedFrame(native.Width, native.Height, native.FrameIndex, native.PresentationTicks, rgba, planes, usesCpuFallback: native.DecodePath != 1u, isYCoCg: native.TextureFormat == 0x0Fu));
                    }
                    finally
                    {
        ...
        private Result Call(Func<int> call, string code)
        {
            ...
            lock (_gate)
            {
                ...
                {
                    var status = call();
                    return status == 0 ? Result.Success() : Failure(code, "The Hap native plugin rejected the operation.");
                }
