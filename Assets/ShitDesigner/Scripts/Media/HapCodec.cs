...
namespace ShitDesigner.Media {
	...
	public sealed class HapMovie {
		...
		private HapMovie(byte[] file, VideoCodec codec, int width, int height, uint timeScale, ulong durationTicks, List<HapMovieSample> samples) { _file = file; Codec = codec; Width = width; Height = height; TimeScale = timeScale; DurationTicks = durationTicks; Samples = samples; }
