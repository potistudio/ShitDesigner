...
namespace ShitDesigner.Application {
	...
	public sealed class ApplicationControlRuntimeReadModel {
		...
		public ApplicationControlRuntimeReadModel(float value, bool hasValue, bool isFiring) { Value = value; HasValue = hasValue; IsFiring = isFiring; }
	}
	...
	public sealed class ApplicationNodeCatalogPortMetadata {
		...
		public ApplicationNodeCatalogPortMetadata(string id, string displayName, string direction, string valueType, bool isRequired) { Id = id ?? string.Empty; DisplayName = displayName ?? string.Empty; Direction = direction ?? string.Empty; ValueType = valueType ?? string.Empty; IsRequired = isRequired; }
	}
	...
	public sealed class ApplicationNodeCatalogParameterMetadata {
		...
		public ApplicationNodeCatalogParameterMetadata(string id, string displayName, string valueType, string defaultValue, string hardRange, bool runtimeStateful,
			string group = null, int order = 0, string description = null, string unit = null, double step = 0d, bool isReadOnly = false, bool isVisible = true) { Id = id ?? string.Empty; DisplayName = displayName ?? string.Empty; ValueType = valueType ?? string.Empty; DefaultValue = defaultValue ?? string.Empty; HardRange = hardRange ?? string.Empty; RuntimeStateful = runtimeStateful; Group = group ?? string.Empty; Order = order; Description = description ?? string.Empty; Unit = unit ?? string.Empty; Step = step; IsReadOnly = isReadOnly; IsVisible = isVisible; }
	}
	...
	public sealed class ApplicationGraphNodeReadModel {
		...
		public ApplicationGraphNodeReadModel(string id, string typeId, string displayName, float x, float y,
			string status = "Ready", bool isPending = false, string statusReason = null, bool enabled = true, string unknownOriginalTypeId = null, int unknownOriginalSchemaVersion = 0, string opaqueRawState = null) { Id = id ?? string.Empty; TypeId = typeId ?? string.Empty; DisplayName = displayName ?? typeId ?? string.Empty; X = x; Y = y; Status = status ?? string.Empty; IsPending = isPending; StatusReason = statusReason ?? string.Empty; Enabled = enabled; UnknownOriginalTypeId = unknownOriginalTypeId ?? string.Empty; UnknownOriginalSchemaVersion = unknownOriginalSchemaVersion; OpaqueRawState = opaqueRawState ?? string.Empty; }
	}
	...
	public sealed class ApplicationGraphPortReadModel {
		...
		public ApplicationGraphPortReadModel(string stableId, string nodeId, string portId, string valueType, string direction, bool isRequired, bool isConnected) { StableId = stableId ?? string.Empty; NodeId = nodeId ?? string.Empty; PortId = portId ?? string.Empty; ValueType = valueType ?? string.Empty; Direction = direction ?? string.Empty; IsRequired = isRequired; IsConnected = isConnected; }
	}
	...
	public sealed class ApplicationGraphConnectionReadModel {
		...
		public ApplicationGraphConnectionReadModel(string id, string fromNodeId, string fromPortId, string toNodeId, string toPortId, bool isImplicitConversion = false, string conversionLabel = null) { Id = id ?? string.Empty; FromNodeId = fromNodeId ?? string.Empty; FromPortId = fromPortId ?? string.Empty; ToNodeId = toNodeId ?? string.Empty; ToPortId = toPortId ?? string.Empty; IsImplicitConversion = isImplicitConversion; ConversionLabel = conversionLabel ?? string.Empty; }
	}
	...
	public sealed class ApplicationGraphReadModel {
		...
		public ApplicationGraphReadModel(IEnumerable<ApplicationGraphNodeReadModel> nodes = null, IEnumerable<ApplicationGraphPortReadModel> ports = null, IEnumerable<ApplicationGraphConnectionReadModel> connections = null) { Nodes = Copy(nodes); Ports = Copy(ports); Connections = Copy(connections); }
	}
	...
	public sealed class ApplicationParameterComponentRangeReadModel {
		...
		public ApplicationParameterComponentRangeReadModel(string name, string minimum, string maximum) { Name = name ?? string.Empty; Minimum = minimum ?? string.Empty; Maximum = maximum ?? string.Empty; }
	}

	public sealed class ApplicationParameterOptionReadModel {
		public string Id { get; }
		public string DisplayName { get; }
		public ApplicationParameterOptionReadModel(string id, string displayName) { Id = id ?? string.Empty; DisplayName = displayName ?? id ?? string.Empty; }
	}
	...
	public sealed class ApplicationDashboardWidgetReadModel {
		...
		public ApplicationDashboardWidgetReadModel(string id, string nodeId, string parameterId, int column, int row, int width, int height, string label, bool isBroken, string brokenReason) { Id = id ?? string.Empty; NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; Column = column; Row = row; Width = width; Height = height; Label = label ?? string.Empty; IsBroken = isBroken; BrokenReason = brokenReason ?? string.Empty; }
	}
	...
	public sealed class ApplicationDashboardReadModel {
		...
		public ApplicationDashboardReadModel(string id, string name, IEnumerable<string> widgetIds = null, IEnumerable<ApplicationDashboardWidgetReadModel> widgets = null) { Id = id ?? string.Empty; Name = name ?? id ?? string.Empty; WidgetIds = Copy(widgetIds); Widgets = Copy(widgets); }
	}
	...
	public sealed class ApplicationPresetReadModel {
		...
		public ApplicationPresetReadModel(string id, string name, bool isBroken = false, string brokenReason = null, string category = null, int sortIndex = 0, IEnumerable<ApplicationPresetEntryReadModel> entries = null) { Id = id ?? string.Empty; Name = name ?? id ?? string.Empty; IsBroken = isBroken; BrokenReason = brokenReason ?? string.Empty; Category = category ?? string.Empty; SortIndex = sortIndex; Entries = Copy(entries); }
	}
	...
	public sealed class ApplicationPresetEntryReadModel {
		...
		public bool IsBroken { get; }
		public string BrokenReason { get; }
		public ApplicationPresetEntryReadModel(string nodeId, string parameterId, string valueType, string value, bool isBroken, string brokenReason) { NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; ValueType = valueType ?? string.Empty; Value = value ?? string.Empty; IsBroken = isBroken; BrokenReason = brokenReason ?? string.Empty; }
	}
	...
	public sealed class ApplicationMediaReadModel {
		...
		public string BrokenReason { get; }
		public bool IsBroken { get; }
		public ApplicationMediaReadModel(string id, string relativePath, long size, string integrityHash, string status = "Ready", string kind = null, string colorSpace = null, string alphaMode = null, int referenceCount = 0, string brokenReason = null) { Id = id ?? string.Empty; RelativePath = relativePath ?? string.Empty; Size = size; IntegrityHash = integrityHash ?? string.Empty; Status = status ?? string.Empty; Kind = kind ?? string.Empty; ColorSpace = colorSpace ?? string.Empty; AlphaMode = alphaMode ?? string.Empty; ReferenceCount = referenceCount; BrokenReason = brokenReason ?? string.Empty; IsBroken = !string.Equals(Status, "Ready", StringComparison.OrdinalIgnoreCase); }
	}
	...
	public sealed class ApplicationOutputSurfaceReadModel {
		...
		public string StatusReason { get; }
		public ApplicationOutputSurfaceReadModel(string id, string targetKind, string state, int width, int height, string fitMode, string backgroundMode, string quality, bool isDemanded, bool isHoldingLastFrame, string statusReason = null, bool isFocused = false) { Id = id ?? string.Empty; TargetKind = targetKind ?? string.Empty; State = state ?? string.Empty; Width = width; Height = height; FitMode = fitMode ?? string.Empty; BackgroundMode = backgroundMode ?? string.Empty; Quality = quality ?? string.Empty; IsDemanded = isDemanded; IsFocused = isFocused; IsHoldingLastFrame = isHoldingLastFrame; StatusReason = statusReason ?? string.Empty; }
	}
	...
	public sealed class ApplicationDiagnosticReadModel {
		...
		public ApplicationDiagnosticReadModel(string entryId, string severity, string code, string message, string nodeId = null, long count = 1, ulong firstFrame = 0, ulong lastFrame = 0) { EntryId = entryId ?? string.Empty; Severity = severity ?? string.Empty; Code = code ?? string.Empty; Message = message ?? string.Empty; NodeId = nodeId ?? string.Empty; Count = Math.Max(1, count); FirstFrame = firstFrame; LastFrame = lastFrame; }
	}
	...
	public sealed class ApplicationDiagnosticsReadModel {
		...
		public ApplicationDiagnosticsReadModel(IEnumerable<ApplicationDiagnosticReadModel> current, IEnumerable<ApplicationDiagnosticReadModel> history, IDictionary<string, long> summary, ReadModelChangeSet<ApplicationDiagnosticReadModel> changeSet) { Current = Copy(current); History = Copy(history); Summary = new ReadOnlyDictionary<string, long>(new Dictionary<string, long>(summary ?? new Dictionary<string, long>(), StringComparer.Ordinal)); ChangeSet = changeSet; }
	}
	...
	public sealed class ApplicationTaskReadModel {
		...
		public ApplicationTaskReadModel(Guid taskId, string kind, string stage, string status, string path = null, Diagnostic diagnostic = null, int completedItems = 0, int totalItems = 0, string currentItem = null) { TaskId = taskId; Kind = kind ?? string.Empty; Stage = stage ?? string.Empty; Status = status ?? string.Empty; Path = path ?? string.Empty; CompletedItems = completedItems; TotalItems = totalItems; CurrentItem = currentItem ?? string.Empty; Diagnostic = diagnostic; }
	}
	...
	public sealed class ApplicationFrameCommandResult {
		...
		public Diagnostic Diagnostic { get; }
		public ApplicationFrameCommandResult(string requestId, ApplicationCommandStatus status, Diagnostic diagnostic = null) { RequestId = requestId ?? string.Empty; Status = status; Diagnostic = diagnostic; }
	}
	...
	public sealed class ApplicationFrameResult {
		...
		public ApplicationFrameResult(ulong frameNumber, bool succeeded, IEnumerable<ApplicationFrameCommandResult> commandResults = null) { FrameNumber = frameNumber; Succeeded = succeeded; CommandResults = Copy(commandResults); }
	}
	...
	public sealed class ApplicationShortcutMetadata {
		...
		public ApplicationShortcutMetadata(ApplicationShortcutCommand command, string displayName, string tooltip, string chord, bool global = true) { Command = command; DisplayName = displayName ?? string.Empty; Tooltip = tooltip ?? string.Empty; Chord = chord ?? string.Empty; Global = global; }
	}
	...
	public sealed class ApplicationParameterEditRequest {
		...
		public ApplicationParameterEditRequest(string nodeId, string parameterId, ParameterValue value, Guid? interactionId = null) { NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; Value = value; InteractionId = interactionId ?? Guid.Empty; }
	}
	...
	public sealed class ApplicationControlMappingRequest {
		...
		public ApplicationControlMappingRequest(string physicalId, string controlPath, float rawMin = 0, float rawMax = 1, bool invert = false, ApplicationPhysicalInputKind kind = ApplicationPhysicalInputKind.Keyboard) { Kind = kind; PhysicalId = physicalId ?? string.Empty; ControlPath = controlPath ?? string.Empty; RawMin = rawMin; RawMax = rawMax; Invert = invert; }
	}
	...
	public sealed class ApplicationLogicalControlRequest {
		...
		public ApplicationLogicalControlRequest(string id, string name, ApplicationLogicalControlKind kind, float initialValue = 0, string presetId = null, IEnumerable<ApplicationControlMappingRequest> mappings = null) { Id = id ?? string.Empty; Name = name ?? string.Empty; Kind = kind; InitialValue = initialValue; PresetId = presetId ?? string.Empty; Mappings = Copy(mappings); }
	}
	...
	public sealed class ApplicationMediaAssetRequest {
		...
		public string ColorSpace { get; }
		public string AlphaMode { get; }
		public ApplicationMediaAssetRequest(string id, string displayName, string relativePath, long byteSize, string integrityHash, string kind = "Experimental", string colorSpace = "SRgb", string alphaMode = "Opaque") { Id = id ?? string.Empty; DisplayName = displayName ?? string.Empty; RelativePath = relativePath ?? string.Empty; ByteSize = byteSize; IntegrityHash = integrityHash ?? string.Empty; Kind = kind ?? string.Empty; ColorSpace = colorSpace ?? string.Empty; AlphaMode = alphaMode ?? string.Empty; }
	}
	...
	public sealed class ApplicationPresetEntryRequest {
		...
		public string ParameterId { get; }
		public ParameterValue Value { get; }
		public ApplicationPresetEntryRequest(string nodeId, string parameterId, ParameterValue value) { NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; Value = value; }
	}
	...
	public sealed class ApplicationPresetRequest {
		...
		public ApplicationPresetRequest(string id, string name, string category = null, int sortIndex = 0, IEnumerable<ApplicationPresetEntryRequest> entries = null) { Id = id ?? string.Empty; Name = name ?? string.Empty; Category = category ?? string.Empty; SortIndex = sortIndex; Entries = Copy(entries); }
	}
	...
	public sealed class ApplicationLogicalControlTargetRequest {
		...
		public bool Invert { get; }
		public ApplicationLogicalControlTargetRequest(string nodeId, string parameterId, ParameterValue targetMin, ParameterValue targetMax, bool invert = false) { NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; TargetMin = targetMin; TargetMax = targetMax; Invert = invert; }
	}
	...
	public sealed class ApplicationExpressionDraft {
		...
		public ApplicationExpressionDraft(string nodeId, string parameterId, ApplicationExpressionKind kind, string logicalControlId = null, ApplicationExpressionDraft left = null, ApplicationExpressionDraft right = null, ParameterValue? outputMinimum = null, ParameterValue? outputMaximum = null) { NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; Kind = kind; LogicalControlId = logicalControlId ?? string.Empty; Left = left; Right = right; OutputMinimum = outputMinimum; OutputMaximum = outputMaximum; }
	}
	...
	public sealed class ApplicationMediaImportRequest {
		...
		public string ColorSpace { get; }
		public string AlphaMode { get; }
		public ApplicationMediaImportRequest(string sourcePath, string displayName, string kind = "Experimental", string colorSpace = "SRgb", string alphaMode = "Opaque") { SourcePath = sourcePath ?? string.Empty; DisplayName = displayName ?? string.Empty; Kind = kind ?? string.Empty; ColorSpace = colorSpace ?? string.Empty; AlphaMode = alphaMode ?? string.Empty; }
	}
	...
	public sealed class ApplicationMediaReferenceReadModel {
		...
		public string ParameterId { get; }
		public bool IsBroken { get; }
		public ApplicationMediaReferenceReadModel(string assetId, string ownerKind, string ownerId, string parameterId, bool isBroken) { AssetId = assetId ?? string.Empty; OwnerKind = ownerKind ?? string.Empty; OwnerId = ownerId ?? string.Empty; ParameterId = parameterId ?? string.Empty; IsBroken = isBroken; }
	}
	...
	public sealed class ApplicationDashboardWidgetRequest {
		...
		public int Height { get; }
		public string Label { get; }
		public ApplicationDashboardWidgetRequest(string widgetId, string nodeId, string parameterId, int column = 0, int row = 0, int width = 1, int height = 1, string label = null) { WidgetId = widgetId ?? string.Empty; NodeId = nodeId ?? string.Empty; ParameterId = parameterId ?? string.Empty; Column = column; Row = row; Width = width; Height = height; Label = label ?? string.Empty; }
	}
	public sealed class ApplicationDashboardPageRequest {
		...
		public ApplicationDashboardPageRequest(string pageId, string name, IEnumerable<ApplicationDashboardWidgetRequest> widgets = null) { PageId = pageId ?? string.Empty; Name = name ?? string.Empty; Widgets = Copy(widgets); }
	}
	public sealed class ApplicationPreviewSettingsRequest {
		...
		public ApplicationPreviewSettingsRequest(string previewId, ApplicationOutputFitMode fitMode = ApplicationOutputFitMode.Fit, string backgroundMode = "Black", string quality = "Project", bool holdLastFrame = true) { PreviewId = previewId ?? string.Empty; FitMode = fitMode; BackgroundMode = backgroundMode ?? string.Empty; Quality = quality ?? string.Empty; HoldLastFrame = holdLastFrame; }
	}
	public sealed class ApplicationOutputDemandRequest {
		...
		public ApplicationOutputDemandRequest(string previewId, string portId = "image", int width = 640, int height = 360, bool focused = false) { PreviewId = previewId ?? string.Empty; PortId = portId ?? string.Empty; Width = width; Height = height; Focused = focused; }
	}
	...
	public sealed class ApplicationReadModelChangeSets {
		...
		public ApplicationReadModelChangeSets(ReadModelChangeSet<ApplicationGraphNodeReadModel> graphNodes, ReadModelChangeSet<ApplicationGraphConnectionReadModel> graphConnections, ReadModelChangeSet<ApplicationParameterReadModel> parameters, ReadModelChangeSet<ApplicationDiagnosticReadModel> diagnostics) { GraphNodes = graphNodes; GraphConnections = graphConnections; Parameters = parameters; Diagnostics = diagnostics; }
