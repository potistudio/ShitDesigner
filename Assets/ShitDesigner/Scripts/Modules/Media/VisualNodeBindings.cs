...
namespace ShitDesigner.Media {
	public interface IVideoFrameConversionPass : IDisposable {
		UnitResult<Diagnostic> Convert(Texture source, RenderTexture target, VideoFrameConversionMetadata metadata);
	}
	...
	public sealed class UnityVideoFrameConversionPass : IVideoFrameConversionPass {
		...
		public UnitResult<Diagnostic> Convert(Texture source, RenderTexture target, VideoFrameConversionMetadata metadata) {
	...
	public sealed class VideoOutputSurfaceFrameAdapter : IVideoOutputSurfaceFrameAdapterWithConversion, IDisposable {
		...
		public Result<IRuntimeImageFrame, Diagnostic> Create(object borrowedTexture, IRuntimeOutputSurface preparedSurface, ulong frameNumber)
			=> Create(borrowedTexture, preparedSurface, frameNumber, null);

		public Result<IRuntimeImageFrame, Diagnostic> Create(object borrowedTexture, IRuntimeOutputSurface preparedSurface, ulong frameNumber, VideoFrameConversionMetadata metadata) {
			...
			try {
				...
				if (converted.HasValue && converted.Value.IsFailure) return Result.Failure<IRuntimeImageFrame, Diagnostic>(converted.Value.Error);
				if (preparedSurface is IRuntimeOutputSurfaceCompletion completion) completion.MarkRendered();
				return Result.Success<IRuntimeImageFrame, Diagnostic>(new MediaRuntimeImageFrame(preparedSurface, frameNumber));
			}
			catch (Exception exception) {
				return Result.Failure<IRuntimeImageFrame, Diagnostic>(new Diagnostic(new DiagnosticCode("media.frame.blit_failed"), Severity.Error, exception.Message, module: "media", exception: DiagnosticExceptionInfo.FromException(exception)));
			}
		}
		...
		private static Result<IRuntimeImageFrame, Diagnostic> Failure(string code, string message) => Result.Failure<IRuntimeImageFrame, Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, module: "media"));
	...
	public sealed class VideoPlayerVisualNodeBinding : IRuntimeVisualNodeBinding {
		...
		public Result<IRuntimeNode, Diagnostic> Create(RuntimeNodeCreateInfo node, ulong generationId) {
			...
			if (asset != null && asset.Value.Type == ParameterType.MediaAssetReference && asset.Value.AsMediaAsset().HasValue) {
				...
				if (request.IsFailure) return Result.Failure<IRuntimeNode, Diagnostic>(request.Error);
				var selected = VideoBackendSelector.Select(request.Value.Probe, _graphics);
				if (selected.IsFailure) return Result.Failure<IRuntimeNode, Diagnostic>(selected.Error);
				backendKind = selected.Value;
			}
			var backend = _backendFactory.Create(node.Id, generationId, backendKind);
			if (backend.IsFailure) return Result.Failure<IRuntimeNode, Diagnostic>(backend.Error);
			try {
				var session = new VideoPlaybackSession(node.Id, generationId, backend.Value);
				return Result.Success<IRuntimeNode, Diagnostic>(new VideoPlayerRuntimeNode(node.Id, generationId, session, new VideoTransportState(), _frameAdapter, prepareResolver: _resolver, backendFactory: _backendFactory, graphics: _graphics));
			}
		...
		private Result<IRuntimeNode, Diagnostic> FailureNode(string code, string message, RuntimeNodeCreateInfo node, ulong generationId, Exception exception = null) =>
			Result.Failure<IRuntimeNode, Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, nodeId: node?.Id ?? default(NodeInstanceId), nodeTypeId: TypeId, generationId: generationId, module: "media", exception: exception == null ? null : DiagnosticExceptionInfo.FromException(exception)));
	}
}
