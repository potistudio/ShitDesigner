...
namespace ShitDesigner.Rendering {
	...
	public static class RenderingFormatPolicy {
		public static UnitResult<Diagnostic> ValidateInternalFormat(ProgramDynamicRange range, IRenderingPlatformCapabilityPort capabilities) {
			...
			if (!capabilities.IsFormatSupported(format, GraphicsFormatUsage.Render)
				...
				return UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode("rendering.format.unsupported"), Severity.Fatal,
