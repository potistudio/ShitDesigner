...
namespace ShitDesigner.Rendering {
	...
	public static class ResolutionDemandIntegrator {
		public static Result<ResolutionDemandResult, Diagnostic> Merge(IEnumerable<ResolutionDemand> demands) {
			var list = (demands ?? Enumerable.Empty<ResolutionDemand>()).ToList();
			if (list.Count == 0)
				return Result.Failure<ResolutionDemandResult, Diagnostic>(new Diagnostic(new DiagnosticCode("rendering.demand.empty"), Severity.Error, "At least one resolution demand is required."));
			var winner = list
			...
			return Result.Success<ResolutionDemandResult, Diagnostic>(new ResolutionDemandResult(new Vector2Int(width, height), winner));
		}
