...
namespace ShitDesigner.Persistence
{
    ...
    public static class ProjectSerializer
    {
        ...
        {
            Id = node.Id.Value, TypeId = node.IsUnknown ? node.Unknown.OriginalNodeTypeId.Value : node.TypeId.Value, SchemaVersion = node.IsUnknown ? node.Unknown.OriginalSchemaVersion : node.SchemaVersion, DisplayName = node.DisplayName, Enabled = node.Enabled, X = node.Position.X, Y = node.Position.Y, SystemOwned = node.SystemOwned, UserAddable = node.UserAddable, RawState = node.IsUnknown ? node.Unknown.RawJsonValue : node.RawState,
    ...
    public sealed class MediaAssetImportProgress
    {
        ...
        internal MediaAssetImportProgress(MediaAssetImportTransactionStage stage, MediaAssetRecord asset = null, Diagnostic diagnostic = null)
        { Stage = stage; Asset = asset; Diagnostic = diagnostic; }
    }
    ...
    public sealed class PendingMediaDeletion
    {
        ...
        internal PendingMediaDeletion(MediaAssetId assetId, string relativePath, string projectRoot)
        { AssetId = assetId; RelativePath = relativePath; ProjectRoot = projectRoot; }
    }
