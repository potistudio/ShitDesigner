...
namespace ShitDesigner.Presentation
{
    ...
    public sealed class PlatformPathRequest
    {
        ...
        public PlatformPathRequest(Guid requestId, Guid projectSessionId, PlatformPathRequestKind kind, string title)
        { RequestId = requestId; ProjectSessionId = projectSessionId; Kind = kind; Title = title ?? string.Empty; }
    }
    ...
    public sealed class PlatformDropResult
    {
        public Guid ProjectSessionId { get; }
        public IReadOnlyList<string> AbsolutePaths { get; }
        public PlatformDropResult(Guid projectSessionId, IEnumerable<string> absolutePaths)
        { ProjectSessionId = projectSessionId; AbsolutePaths = new ReadOnlyCollection<string>((absolutePaths ?? Enumerable.Empty<string>()).Where(x => !string.IsNullOrWhiteSpace(x)).ToList()); }
    }
    ...
    public sealed class PresentationNoticeSink : IPresentationNoticeSink
    {
        ...
        public void Record(PresentationSeverity severity, string code, string message, string panelId = null)
        { _notices.Add(new PresentationNotice(Guid.NewGuid(), severity, code, message, panelId)); }
    ...
    public sealed class PresentationNotice
    {
        ...
        public PresentationNotice(Guid id, PresentationSeverity severity, string code, string message, string panelId)
        { Id = id; Severity = severity; Code = code ?? string.Empty; Message = message ?? string.Empty; PanelId = panelId ?? string.Empty; }
    }
}
