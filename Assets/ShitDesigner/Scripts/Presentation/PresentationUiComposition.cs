...
namespace ShitDesigner.Presentation {
	...
	public static class PresentationUiComposition {
		...
		private sealed class ParameterGroupBinding {
			...
			public ParameterGroupBinding(Foldout foldout, IEnumerable<ParameterReadModel> parameters, PresentationCoordinator coordinator) { _foldout = foldout; _parameters = (parameters ?? Enumerable.Empty<ParameterReadModel>()).ToList(); _coordinator = coordinator; }
			public void Rebuild() {
		...
		private static void RebuildLibrary(VisualElement panel, IEnumerable<NodeCatalogItem> entries, PresentationCoordinator coordinator) {
			...
			foreach (var item in catalog) {
				...
				var button = new Button(() => coordinator?.Submit("graph.add_node", entry.TypeId,
					new KeyValuePairValue("nodeTypeId", entry.TypeId), new KeyValuePairValue("x", "0"), new KeyValuePairValue("y", "0"))) { text = entry.DisplayName, name = "node-library-" + entry.TypeId.ToLowerInvariant() };
		...
		private sealed class VisualParameterControlFactory : IParameterControlFactory {
			...
			public VisualParameterControlFactory(VisualElement panel, PresentationCoordinator coordinator, ParameterControlKind kind) { _panel = panel; _coordinator = coordinator; _kind = kind; }
				...
				foreach (var widget in page.Widgets) {
					...
					if (item.IsBroken) {
						...
						var widgetButton = new Button(() => coordinator?.Submit("parameter.set_base", item.ParameterId,
							new KeyValuePairValue("nodeId", item.NodeId), new KeyValuePairValue("parameterId", item.ParameterId))) { text = (string.IsNullOrEmpty(item.NodeId) ? item.ParameterId : item.NodeId + " · ") + item.ParameterId };
