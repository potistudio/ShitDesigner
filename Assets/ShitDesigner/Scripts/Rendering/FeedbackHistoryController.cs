...
namespace ShitDesigner.Rendering {
	...
	public sealed class FeedbackHistoryController : IDisposable {
		...
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> EnsureDescriptor(TextureDescriptor descriptor, ulong frameNumber) {
			if (_disposed) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.disposed", "The Feedback history is disposed."));
			if (frameNumber == 0) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.frame_invalid", "Feedback frame number must be positive."));
			if (HasHistory && _hasDescriptor && _descriptor == descriptor) return CSharpFunctionalExtensions.UnitResult.Success<Diagnostic>();
			...
			if (first.IsFailure) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(first.Error);
			var second = _pool.Acquire(descriptor, nextOwner, frameNumber);
			if (second.IsFailure) {
				first.Value.Release(previousOwner, frameNumber);
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(second.Error);
			}
			...
			try {
				...
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode("rendering.feedback.clear_failed"), Severity.Error,
		...
		public CSharpFunctionalExtensions.Result<BorrowedOutputSurface, Diagnostic> BorrowPrevious(ulong frameNumber) {
			if (!HasHistory) return CSharpFunctionalExtensions.Result.Failure<BorrowedOutputSurface, Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.history_missing", "Feedback history has not been prepared."));
			return _previous.Borrow(frameNumber);
		}

		/// <summary>Copies current input into the next buffer, then swaps at the boundary.</summary>
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Commit(ImageFrame currentInput, ulong frameNumber) {
			if (frameNumber == 0) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.frame_invalid", "Feedback frame number must be positive."));
			if (!HasHistory) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.history_missing", "Feedback history has not been prepared."));
			if (currentInput.Texture == null || currentInput.Size.x != _descriptor.Width || currentInput.Size.y != _descriptor.Height || currentInput.ColorFormat != _descriptor.GraphicsFormat)
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.input_mismatch", "Feedback input does not match the history descriptor."));
			if (currentInput.Texture == _next.Texture || currentInput.Texture == _previous.Texture)
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.self_copy", "Feedback input cannot be one of its history surfaces."));
			try {
				Graphics.Blit(currentInput.Texture, _next.Texture);
			}
			catch (Exception exception) {
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode("rendering.feedback.commit_failed"), Severity.Error,
		...
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Reset(ulong frameNumber) {
			if (_disposed) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.disposed", "The Feedback history is disposed."));
			if (frameNumber == 0) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(RenderingDiagnostics.Error("rendering.feedback.frame_invalid", "Feedback frame number must be positive."));
			if (!HasHistory) return CSharpFunctionalExtensions.UnitResult.Success<Diagnostic>();
			try {
				...
				return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode("rendering.feedback.reset_failed"), Severity.Error,
