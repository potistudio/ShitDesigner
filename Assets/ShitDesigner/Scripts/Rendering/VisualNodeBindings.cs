...
namespace ShitDesigner.Rendering {
	...
	public sealed class ShaderMaterialRegistry {
		...
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Register(ShaderMaterialBinding binding) {
		...
		private static CSharpFunctionalExtensions.UnitResult<Diagnostic> Failure(string code, string message) => CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, module: "rendering"));
	}
	...
	public sealed class ShaderVisualNodeBinding : IRuntimeVisualNodeBinding {
		...
		public CSharpFunctionalExtensions.Result<IRuntimeNode, Diagnostic> Create(RuntimeNodeCreateInfo node, ulong generationId) {
			...
			if (!TryResolve(out var binding)) return CSharpFunctionalExtensions.Result.Failure<IRuntimeNode, Diagnostic>(AvailabilityDiagnostic);
			...
			try {
				if ((binding.Passes.Count > 1 || binding.Stateful) && _pool != null && !string.IsNullOrWhiteSpace(_sessionId))
					return CSharpFunctionalExtensions.Result.Success<IRuntimeNode, Diagnostic>(new ShaderPassGraphRuntimeNode(node, generationId, binding, _pool, _sessionId, generator, blend));
				...
				return CSharpFunctionalExtensions.Result.Success<IRuntimeNode, Diagnostic>(new ShaderRuntimeNode(node, generationId, binding, generator, blend));
			}
		...
		private CSharpFunctionalExtensions.Result<IRuntimeNode, Diagnostic> FailureNode(string code, string message, RuntimeNodeCreateInfo node, ulong generationId, Exception exception = null) =>
			CSharpFunctionalExtensions.Result.Failure<IRuntimeNode, Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, nodeId: node?.Id ?? default(NodeInstanceId), nodeTypeId: TypeId, generationId: generationId, module: "rendering", exception: exception == null ? null : DiagnosticExceptionInfo.FromException(exception)));
	...
	public sealed class FeedbackVisualNodeBinding : IRuntimeVisualNodeBinding, IFeedbackCommitter, IFeedbackResetter, IRuntimeResourcePreparationWithPlan, IDisposable {
		...
		public CSharpFunctionalExtensions.Result<IRuntimeNode, Diagnostic> Create(RuntimeNodeCreateInfo node, ulong generationId) {
			...
			return CSharpFunctionalExtensions.Result.Success<IRuntimeNode, Diagnostic>(runtime);
		}
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Commit(NodeInstanceId nodeId, NodeOutputResult input, FrameSnapshot snapshot) {
		...
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Reset(NodeInstanceId nodeId) {
			if (!_nodes.TryGetValue(nodeId, out var node)) return Failure("rendering.feedback.node_missing", "Feedback node is not registered.");
		...
		public CSharpFunctionalExtensions.UnitResult<Diagnostic> Prepare(FrameSnapshot snapshot, FrameEvaluationContext evaluation) {
			if (!IsAvailable) return CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(AvailabilityDiagnostic);
		...
		private CSharpFunctionalExtensions.Result<IRuntimeNode, Diagnostic> FailureNode(string code, string message, RuntimeNodeCreateInfo node, ulong generationId) => CSharpFunctionalExtensions.Result.Failure<IRuntimeNode, Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, nodeId: node?.Id ?? default(NodeInstanceId), nodeTypeId: TypeId, generationId: generationId, module: "rendering"));
		private static CSharpFunctionalExtensions.UnitResult<Diagnostic> Failure(string code, string message) => CSharpFunctionalExtensions.UnitResult.Failure<Diagnostic>(new Diagnostic(new DiagnosticCode(code), Severity.Error, message, module: "rendering"));
	}
	...
	public sealed class FeedbackRuntimeNode : IRuntimeNode, IRuntimeDemandAwareNode {
		...
		internal CSharpFunctionalExtensions.UnitResult<Diagnostic> Prepare(TextureDescriptor descriptor, ulong frameNumber) => _history.EnsureDescriptor(descriptor, frameNumber);
		internal CSharpFunctionalExtensions.UnitResult<Diagnostic> Commit(NodeOutputResult input, FrameSnapshot snapshot) {
			if (!input.IsAvailable || !input.Value.IsImageFrame) return CSharpFunctionalExtensions.UnitResult.Success<Diagnostic>();
		...
		internal CSharpFunctionalExtensions.UnitResult<Diagnostic> Reset(NodeInstanceId nodeId) => nodeId == NodeId ? _history.Reset(_history.LastCommitFrame == 0 ? 1 : _history.LastCommitFrame + 1) : Failure("rendering.feedback.owner", "Feedback reset owner does not match.");
