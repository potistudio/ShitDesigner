...
namespace ShitDesigner.Scene
{
    ...
    public sealed class SceneRuntimeNode : IRuntimeNode, IRuntimeDemandAwareNode
    {
        ...
        internal SceneRuntimeNode(RuntimeNodeCreateInfo record, ulong generationId, SceneNodeRuntime scene, Action<SceneNodeRuntime, FrameSnapshot> applyEffectiveParameters)
        { _record = record ?? throw new ArgumentNullException(nameof(record)); GenerationId = generationId; _scene = scene ?? throw new ArgumentNullException(nameof(scene)); _applyEffectiveParameters = applyEffectiveParameters; _lastClock = 0d; }
