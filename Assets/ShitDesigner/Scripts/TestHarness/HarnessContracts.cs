    ...
    public sealed class HarnessMetricAccumulator
    {
        ...
        public void CompleteIntervals(double endSeconds)
        {
            foreach (var pair in _intervals.ToList())
            {
                ...
                _completedIntervals.Add(new HarnessDiagnosticInterval { kind = pair.Key, startSeconds = pair.Value.StartSeconds,
                    endSeconds = Math.Max(pair.Value.StartSeconds, endSeconds), durationSeconds = Math.Max(0d, endSeconds - pair.Value.StartSeconds), samples = pair.Value.Samples });
        ...
        private void UpdateInterval(string kind, bool active, double seconds)
        {
            ...
            {
                _completedIntervals.Add(new HarnessDiagnosticInterval { kind = kind, startSeconds = state.StartSeconds, endSeconds = time,
                    durationSeconds = Math.Max(0d, time - state.StartSeconds), samples = state.Samples });
                state.Active = false;
            }
        }
