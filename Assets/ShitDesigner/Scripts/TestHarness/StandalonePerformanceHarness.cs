...
namespace ShitDesigner.TestHarness
{
	...
	public sealed partial class StandalonePerformanceHarness : MonoBehaviour
	{
		...
		private HarnessArtifact BuildPerformanceArtifact()
		{
			...
			{
				sampleSeconds = x.sampleSeconds,
				programFrameNumber = x.programFrameNumber,
				previews = x.previews
			}).ToArray();
			...
			return new HarnessArtifact
			{
				...
				codec = _codec.ToString(),
				corpusVersion = _corpus?.Version ?? string.Empty,
				corpusFile = _corpus?.Entry?.file ?? string.Empty,
				...
				packageVersion = UnityApplication.version,
				...
				{
					updateSamples = _metrics?.Samples.Count ?? 0,
					measuredFrames = _metrics?.PresentedFrames ?? 0,
					presentedFrames = _metrics?.PresentedFrames ?? 0,
					timingAvailableFrames = _metrics?.TimingAvailableFrames ?? 0,
					timingUnavailableFrames = _metrics?.TimingUnavailableFrames ?? 0,
					goodFrameRatio = _metrics?.GoodFrameRatio ?? 0d,
					averageCpuMilliseconds = _metrics?.AverageCpuMilliseconds ?? double.NaN,
					averageGpuMilliseconds = _metrics?.AverageGpuMilliseconds ?? double.NaN,
					maxCpuMilliseconds = _metrics?.MaxCpuMilliseconds ?? double.NaN,
					maxGpuMilliseconds = _metrics?.MaxGpuMilliseconds ?? double.NaN,
					maxConsecutiveProgramMissing = _metrics?.MaxConsecutiveMissing ?? 0,
					minimumProgramCadenceFps = _metrics?.MinimumProgramCadenceFps ?? double.NaN,
					gcAllocatedBytes = _gcAllocatedDuringMeasurement,
					gcCollectionCount0 = Math.Max(0, GC.CollectionCount(0) - _gcCollections0),
					gcCollectionCount1 = Math.Max(0, GC.CollectionCount(1) - _gcCollections1),
					gcCollectionCount2 = Math.Max(0, GC.CollectionCount(2) - _gcCollections2),
				...
				output = new HarnessOutputArtifact
				{
					programWidth = activeProgram?.Width ?? 0,
					programHeight = activeProgram?.Height ?? 0,
					programFormat = activeProgram?.GraphicsFormat ?? string.Empty,
					programTargetFps = activeProgram?.TargetFramesPerSecond ?? 0,
					programState = program?.State ?? string.Empty,
					previewCount = previewDescriptors.Length,
				...
				resources = new HarnessResourceArtifact
				{
					poolBudgetBytes = before?.TexturePool?.BudgetBytes ?? 0,
					poolCurrentBytes = poolCurrentBytes,
					poolLeasedBytes = before?.TexturePool?.LeasedBytes ?? 0,
					poolFreeBytes = before?.TexturePool?.FreeBytes ?? 0,
					poolHighWaterBytes = before?.TexturePool?.HighWaterBytes ?? 0,
					sceneCount = before?.SceneCount ?? 0,
					layerCount = before?.LayerCount ?? 0,
					backendCount = before?.BackendCount ?? 0,
					nativeContextCount = before?.NativeContextCount ?? 0,
					activeOutputLeases = before?.ActiveOutputLeaseCount ?? 0,
					poolEntryCount = before?.TexturePool?.Entries.Count ?? 0,
					endLeases = endLeases + endPoolEntries,
					endPoolEntryCount = endPoolEntries,
					endActiveOutputLeases = endLeases,
					endSceneCount = after?.SceneCount ?? 0,
					endLayerCount = after?.LayerCount ?? 0,
					endBackendCount = after?.BackendCount ?? 0,
					endNativeContextCount = after?.NativeContextCount ?? 0
				},
