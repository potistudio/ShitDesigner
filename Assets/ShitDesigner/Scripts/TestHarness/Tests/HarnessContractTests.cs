	...
	public sealed class HarnessContractTests
	{
		...
		public void AcceptanceContract_RecoveryRequiresKnownBackupFingerprintAndRecoveryFileAccess()
		{
			var artifact = new HarnessAcceptanceArtifact
			{
				...
				presentationRootAvailable = true,
				programAndPreviewsReady = true,
				fileProjectReadable = false,
				fileProjectWritable = true,
				backupFileReadable = true,
				persistence = new HarnessAcceptancePersistenceArtifact
				{
					recovered = true,
					dirtyAfterRecovery = true,
					mainFilePreservedAfterRecovery = true,
					fingerprint = "loaded-from-backup",
					expectedFingerprint = "different",
					backupFingerprint = "loaded-from-backup",
					expectedBackupFingerprint = "different"
				}
			};
		...
		public void AcceptanceContract_RejectsStaleCodecFrameEvenWhenNodeIsReady()
		{
			...
			var artifact = new HarnessAcceptanceArtifact
			{
				...
				fixtures = codecs.Select(codec => new HarnessAcceptanceFixtureArtifact
				{
					...
					frameBefore = 10,
					frameAfter = 10,
					previewFrameBefore = 10,
					previewFrameAfter = 11,
					frameReady = true
				}).ToArray()
			};
			Assert.That(AcceptanceContract.ValidateStage(HarnessAcceptanceStage.Initial, artifact), Does.Contain("fixture"));
			...
			try
			{
				var artifact = new HarnessArtifact
				{
					...
					timing = new HarnessTimingArtifact
					{
						...
						measuredFrames = 2,
						presentedFrames = 2,
						timingAvailableFrames = 1,
						timingUnavailableFrames = 1,
						...
						frameTimingSource = new HarnessFrameTimingSourceArtifact
						{
							rawCount = 16,
							rawIdentity = 42d,
							rawCpuMilliseconds = double.NaN,
							rawGpuMilliseconds = 0d,
							pendingBefore = 16,
							pendingAfter = 15,
							outcome = "RawInvalid",
							candidateOutcome = "RawInvalid",
							performanceFrameNumber = 41UL,
							exceptionType = ""
						},
					...
					acceptance = new HarnessAcceptanceArtifact
					{
						...
						{
							taskAfterId = "save-task",
							taskAfterKind = "Save",
							taskAfterStage = "Failed",
							taskAfterStatus = "Failed",
							taskAfterPath = "C:/acceptance/project",
							taskAfterDiagnosticCode = "persistence.json_invalid",
							taskAfterDiagnosticMessage = "readback\nrejected",
							taskAfterExceptionType = "System.IO.InvalidDataException",
							taskAfterExceptionMessage = "required value\nwas null",
							taskAfterExceptionStackTrace = "stack\ntrace"
						},
						fixtures = new[]
						{
						...
						{
							frameNumber = 42,
							programState = "HoldingLastFrame",
							programWidth = 1920,
							programHeight = 1080,
							programReason = "fixture transition",
							previews = new[]
							{
		...
		private static HarnessMetricSample ValidSample(int index, bool healthy = true, ulong? frame = null)
		{
			return new HarnessMetricSample
			{
				cpuMilliseconds = 8,
				gpuMilliseconds = 7,
				sampleSeconds = index / 60d,
				programFrameNumber = frame ?? (ulong)(index + 1),
				programWidth = 1920,
				programHeight = 1080,
		...
		private static HarnessPreviewMetric PreviewSample(string id, int stage, int frame)
		{
			...
			return new HarnessPreviewMetric
			{
				...
				targetFramesPerSecond = fps[stage],
				frameNumber = (ulong)frame,
				quality = "Stage" + stage,
				qualityStage = stage
			};
		}
