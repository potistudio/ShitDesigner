...
namespace ShitDesigner.Main.Tests {
	public sealed class NativeOutputMenuTests {
		...
		public void LiveGraphUsesTheHardCodedOutputResolutions() {
			...
			Assert.That(sizes.Program.Width, Is.EqualTo(1560));
		...
		public void ExternalDisplaysUseTheHardCodedOutputResolutions() {
			Assert.That(LiveExternalDisplayOutput.ResolveOutputResolution(2), Is.EqualTo(new Vector2Int(1560, 854)));
		...
		public void ControllerReflectsAvailabilityAndDispatchesNativeCommands() {
			...
			using (var controller = new LiveOutputMenuController(target, backend)) {
				...
				Assert.That(backend.LastState.EmulationAspect, Is.EqualTo(ExternalDisplayEmulationAspect.Ratio4_5x1));
				...
				Assert.That(target.EmulationChangeCount, Is.EqualTo(4));
			}
