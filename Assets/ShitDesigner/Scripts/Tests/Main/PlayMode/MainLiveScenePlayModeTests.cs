...
namespace ShitDesigner.Main.Tests {
	public sealed class MainLiveScenePlayModeTests {
		...
		public IEnumerator MainBootsRendersAndSwitchesItsAuthoredLiveGraph() {
			...
			var effectPatchControls = ui.Q<ScrollView>("effect-patch-controls");
			...
			Assert.That(effectPatchControls, Is.Not.Null);
			...
			Assert.That(effectPatchControls.horizontalScrollerVisibility, Is.EqualTo(ScrollerVisibility.Hidden));
			...
			Assert.That(effectPatchControls.mode, Is.EqualTo(ScrollViewMode.Vertical));
			...
			host.MoveCatalogSelection(1, 0);
			yield return null;
			Assert.That(host.ReadModel.SelectedCatalogRole, Is.EqualTo(LivePatchRole.Effect));
			Assert.That(host.ReadModel.SelectedCatalogPatchId, Is.Empty);
