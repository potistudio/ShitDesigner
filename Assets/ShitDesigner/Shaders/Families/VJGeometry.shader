Shader "Hidden/ShitDesigner/VJ/Geometry"
{
	Properties
	{
		...
		_VJDisplacementTex ("Displacement", 2D) = "black" {}
	...
	SubShader
	{
		...
		float _SD_PassIndex;
		float _SD_PassCount;
		#include "../Includes/VJGeometry.hlsl"
		float4 VJGeometryGraphFragment(v2f i) : SV_Target
		{
			float2 uv = saturate(i.uv);
			int variant = clamp((int)round(_VJVariant), 0, 41);
			int stage = clamp((int)round(_SD_PassIndex), 0, 1);
			float4 current = VJFinite4(tex2D(_MainTex, uv));
			if (stage == 0)
			{
				return VJFinite4(VJGeometryEvaluate(_MainTex, _VJDisplacementTex, uv, variant));
			}
			float4 source = VJFinite4(tex2D(_SD_SourceTex, uv));
			float4 history = VJFinite4(tex2D(_HistoryTex, uv));
			float historyWeight = variant >= 39 ? 0.45 : 0.0;
			float4 state = lerp(current, history, historyWeight);
			// Flow/datamosh/fluid variants consume history on the second
			// stage; spatial warps use the original input as a final blend.
			return VJFinite4(lerp(state, source, variant >= 39 ? 0.04 : 0.12));
		}
		ENDCG
		Pass
		{
			...
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment VJGeometryGraphFragment
			ENDCG
		}
		Pass
		{
			Name "GeometryStateComposite"
			ZTest Always Cull Off ZWrite Off
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment VJGeometryGraphFragment
