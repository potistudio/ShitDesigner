...
$harnessContractSource = Read-Source 'Assets/ShitDesigner/Scripts/TestHarness/Tests/HarnessContractTests.cs'
$performanceContractSource = Read-Source 'Assets/ShitDesigner/Scripts/TestHarness/Tests/PerformanceHarnessContractTests.cs'
$acceptanceContractSource = Read-Source 'Assets/ShitDesigner/Scripts/TestHarness/Tests/StandaloneAcceptanceContractTests.cs'
$acceptanceHarnessSource = Read-Source 'Assets/ShitDesigner/Scripts/TestHarness/StandaloneAcceptanceHarness.cs'
...
$scriptingBackendBlock = [regex]::Match($projectSettings, '(?ms)^[ \t]{2}scriptingBackend:[ \t]*\r?\n(?<block>(?:(?:[ \t]{4,}[^\r\n]*)(?:\r?\n|$))+?)(?=^[ \t]{2}\S|\z)')
Assert-Contract ($scriptingBackendBlock.Success -and $scriptingBackendBlock.Groups['block'].Value -match '(?m)^[ \t]*Standalone:[ \t]*1[ \t]*$') 'Product Standalone Player must permanently use the IL2CPP scripting backend.'
