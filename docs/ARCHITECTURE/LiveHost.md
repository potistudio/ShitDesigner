...
- 編集モード中の、InspectorでCueへ割り当てたキーまたは対応するCueボタンは、選択中のFXノード型を対象Cueへ割り当てる。通常時のINSTANT FX発火には使用しない。
- `Shift`とCueへ割り当てたキーの組み合わせは、モードにかかわらず対象Cueを発火・再割り当てせず、そのCueのFXパラメーターへライブパラメーター表示を切り替える。未割り当てCueでは変更しない。
