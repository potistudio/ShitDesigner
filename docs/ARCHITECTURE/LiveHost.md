...
### 2026-08-29: BPMクロックのビート位置を合わせ直せる

- ビート位置の合わせ直し量はミリ秒単位のグローバル要求として受け付け、範囲は `-1000` から `1000` とする。
- 入力時点のBPMでビート位相へ変換して保持するため、後からBPMを変更しても合わせ直し位置は動かない。UIには最後に指定したミリ秒値を表示する。
- 正の補正はビートを遅らせ、累積ビート自体は変更しない。`BeatClockFrame` は累積ビートと調整後のビート位置を分離して公開し、位相とビート境界を使うSceneの両方へ同じ補正を適用する。
