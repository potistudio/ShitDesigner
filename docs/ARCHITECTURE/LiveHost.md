...
- 左右矢印はMain／Overlay／FXタブを切り替え、上下矢印はタブ内のカタログ選択を移動する。`1`から`8`は、押した時点のビートに対応するOverlayレーンのセルだけをONにする。キーを押し続けても後続ビートへON状態は追従せず、`Shift`の有無で挙動は変わらない。MainまたはOverlayタブではEnterで選択パッチを直接Launchする。FXタブの選択ノードはLaunchしない。
