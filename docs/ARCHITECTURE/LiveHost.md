...
- `PatchDefinition` はInspectorでProgram graphのノード、接続およびノードパラメーターを設定し、Unityの `Scene3DDefinition` をScene Nodesの単純なリストとして参照し、ライブUIへ公開するパラメーターを明示的に束ねる。Shaderパラメーターは必要なものだけを個別に追加し、未追加の値はManifestの既定値を使用する。これはUnityのSceneとは別の論理的なパッチである。
