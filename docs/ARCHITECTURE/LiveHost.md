...
- 左右矢印はMain／Overlay／FXタブを切り替え、上下矢印はタブ内のパッチカタログ選択を移動する。`1`から`8`は選択中のOverlayシーンを対応するシーケンサーレーンへ割り当て、Enterは選択パッチを直接Launchする。
- `QWERTYUIOP` は、選択中のタブ、カタログ項目およびロード中パッチに依存せず、FXカタログの1番から10番を直接Launchするグローバルなインスタントエフェクト入力とする。これらのキーはパッチ固有のKeyboard Inputsには渡さない。
